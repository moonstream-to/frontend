import { Flex, Image, Link, LinkBox, LinkOverlay, chakra } from "@chakra-ui/react";
import { LinkBoxProps } from "@chakra-ui/react";

interface TrustedBadgeProps extends LinkBoxProps {
  name: string;
  caseURL?: string;
  ImgURL: string;
  scaling: number;
...
const TrustedBadge: React.FC<TrustedBadgeProps> = ({
  name,
  caseURL,
  ImgURL,
  scaling,
      ...
      <LinkOverlay href={boxURL} isExternal>
        ...
        >
          ...
          {caseURL && (
            ...
            <Link fontSize={["sm", null, "md", "lg"]} textColor="orange.900" href={caseURL}>
              {`Read more >`}
            </Link>
            // </RouterLink>
          )}
