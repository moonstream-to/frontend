import { AccordionButton, AccordionIcon, GridItem, Flex, Text, Spacer } from "@chakra-ui/react";
      ...
      >
        ...
        <GridItem fontWeight="400" w="100%">
          <Flex mr="auto" justifyContent="start" alignItems="center">
            <GroupImage
              shadowcorns={group.records.length > 2 ? group.records.slice(0, 3) : group.records}
              metadata={metadata}
            />
            <Text pl={["0px", "0px", "7px"]}>{`${group.records.length} Shadowcorns`}</Text>
          </Flex>
          <Spacer />
        </GridItem>
        <GridItem
