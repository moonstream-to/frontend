...
const Airdrop = () => {
  ...
  const getCollections = async (userId: string) => {
    return axios.get(`${ENTITY_API}/public/collections?user_id=${userId}`).then((res: any) => res.data?.collections)
  }
  ...
  const createPublicEntryMutation = useMutation(
    ({ address, email, discord, password }: { address: string; email: string; discord: string; password: string }) => {
      return axios.get(`${ENTITY_API}/public/collections/${collectionId}/search?required_field=address:${address}`).then((res) => {
