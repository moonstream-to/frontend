...
import { Dropper } from "./types/Dropper"
...
export const getClaim = (address: any, ctx: any) => async (claimId: string) => {
	...
	const status = await dropper.methods.getClaimStatus(claimId, ctx.account).call()
...
export const getSignerForClaim = (address: any, ctx: any) => async (claimId: string) => {
	const web3 = ctx.web3
	const dropper = new web3.eth.Contract(dropperAbi) as any as Dropper
	dropper.options.address = address
