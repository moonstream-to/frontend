...
export const useGofpContract = ({
  ...
  const getPathForToken = async (tokenId: number) => {
    const res = await gardenContract.methods
      .getPathChoice(sessionId, tokenId, currentStage.data)
      .call() //TODO current stage
    return Number(res)
  }
  ...
  const setApproval = useMutation(
    async () => {
      ...
      tokenContract.options.address = sessionInfo.data[0]
      return await tokenContract.methods.setApprovalForAll(gardenContractAddress, true).send({
        from: web3ctx.account,
      })
    },
    {
