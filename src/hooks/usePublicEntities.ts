...
const fetchSearchPublicEntity = (queryKey: any) => {
  ...
  return axios.get(
    `${ENTITY_API}/public/collections/${WHITELIST_EVENT_COLLECTION_ID}/search?required_field=address:${searchingAddress}`,
  );
};

export const useSearchPublicEntity = (
  onSuccess: any,
  onError: any,
  searchingAddress: string,
) => {
  return useQuery(
    ['search-public-entity', searchingAddress],
    fetchSearchPublicEntity,
    {
      onSuccess,
      onError,
      select: (data: any) => {
...
const fetchCreatePublicEntity = (queryKey: any) => {
  ...
  return axios.post(
    `${ENTITY_API}/public/collections/${WHITELIST_EVENT_COLLECTION_ID}/entities`,
    data,
  );
};

export const useCreatePublicEntity = (
  onSuccess: any,
  onError: any,
  address: string,
) => {
